...
export class StatisticsComponent {
  ...
  private loadOrdersPerMonth() {
    this.bookService.getOrdersPerMonth(this.yearInput).subscribe(response => {
  ...
  private loadEarningsPerMonth() {
    this.bookService.getEarningsPerMonth(this.yearInput).subscribe(response => {
      const ctx = this.mainChartRef.nativeElement.getContext('2d');
      if (ctx) {
        const data = response.data;
  ...
  private loadPublisherDistribution() {
    this.bookService.getPublisherDistribution().subscribe(response => {
      const ctx = this.mainChartRef.nativeElement.getContext('2d');
      if (ctx) {
        const data = response.data;
